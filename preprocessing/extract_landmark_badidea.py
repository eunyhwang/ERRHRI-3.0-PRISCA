#code from Francesco # path modified for the local one!!!
...
DATASET = "bad"
DATASET = "badidea"
def scan_dataset_from_csv(root, train_csv_path, val_csv_path, extensions=('.mp4', '.avi', '.mov')):
    
    train_folders = set(pd.read_csv(train_csv_path)['participant_id'].astype(str).str.strip())
    val_folders = set(pd.read_csv(val_csv_path)['participant_id'].astype(str).str.strip())

    trainval_root = os.path.join(root, "trainval")
    train_samples, val_samples = [], []

    for folder_name in os.listdir(trainval_root):
        folder_path = os.path.join(trainval_root, folder_name)
        ...
        for fname in os.listdir(folder_path):
            ...
            if "_" not in base_name: 
                continue
            label = int(base_name.rsplit("_", maxsplit=1)[1])
    ...
    print(f"Trovati {len(train_samples)} video in Train")
    print(f"Trovati {len(val_samples)} video in Validation")
...
if __name__ == "__main__":
    CACHE_DIR = "/media/Datasets/bad_idea/trainval_npy_landmark"
    TRAIN_CSV = "/home/ehwang/PRISCA-ERRHRI-3/data/badidea/train_labels.csv"
    VAL_CSV = "/home/ehwang/PRISCA-ERRHRI-3/data/badidea/val_labels.csv"
    DATASET_ROOT = "/media/Datasets/bad_idea/trainval_badidea_dataset/"
    train_samples, val_samples = scan_dataset_from_csv(DATASET_ROOT, TRAIN_CSV, VAL_CSV)
